...
- 따라서 `layoutBody()` 루트 뷰에 폭을 직접 고정합니다. **우선순위는 `.defaultHigh`로 낮춰 주세요** — 뷰 자체의 244pt 상한 제약이 required 우선순위라, 폭 제약도 required(기본)로 걸면 config를 244 이상으로 올렸을 때 두 제약이 충돌해 Auto Layout 경고와 함께 한쪽이 깨집니다.
...
widthConstraint.priority = .defaultHigh  // 뷰 자체 상한(244pt, required)과의 충돌 방지
